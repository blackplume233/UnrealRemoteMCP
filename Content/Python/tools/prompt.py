...
- `get_unreal_state`：确认已连接、UE 版本、当前关卡等（任何任务的第一步）。
...
- `get_unreal_state`：获取引擎环境综合信息，其中 `paths.unreal_api` 字段为 Unreal Python API 索引路径
...
| `slate` | 编辑器 Slate UI 查询与交互：窗口/Widget 树/Tab 列表、鼠标点击、键盘输入、滚轮、焦点管理、通知 | `slate_get_all_windows`, `slate_click_at_position`, `slate_send_key_press`, `slate_invoke_tab`, `slate_show_notification` |

**何时使用 Domain Tools：**
- 需要**查询或操作编辑器 UI**（窗口、Tab、Widget、鼠标点击、键盘输入）→ 使用 `slate` domain
